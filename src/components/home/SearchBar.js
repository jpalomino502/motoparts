...
const FilterSearch = () => {
  ...
  return (
    <div className="mx-auto mb-4 lg:mt-8">
      <div className="bg-gradient-to-r from-[#ff0000] to-[#d70000] rounded-lg shadow-md overflow-hidden">
        ...
        {(showFilters || !isMobile) && (
          <div className={`mt-4 ${isMobile ? 'space-y-4' : 'flex flex-wrap gap-2'}`}>
            {Object.keys(filters).map((filterKey, index) => {
              ...
              return (
                isVisible && (
                  <div key={filterKey} className={`relative bg-white rounded-md ${isMobile ? 'w-full' : 'flex-1'}`}>
                    <select
      ...
      {(!isMobile || showFilters) && (
        <button
          ...
          className="mt-4 w-full bg-black text-white py-2 px-3 rounded-md hover:bg-[#121212] transition-colors duration-300 ease-in-out text-sm font-semibold"
        >
          Buscar filtro
        </button>
      )}
